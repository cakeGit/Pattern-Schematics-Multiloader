...
public class PatternSchematicsToolSelectionScreen extends Screen {
  ...
  public final String scrollToCycle = Component.translatable("create.gui.toolmenu.cycle")
      .getString();
  ...
  private void draw(GuiGraphics graphics, float partialTicks) {
    ...
    if (tools.size() > 1) {
      ...
      int width = minecraft.getWindow()
          .getGuiScaledWidth();
      if (!focused)
        graphics.drawCenteredString(minecraft.font, Component.translatable("create." + holdToFocus, keyName), width / 2,
            y - 10, 0xEECDEE);
      else
